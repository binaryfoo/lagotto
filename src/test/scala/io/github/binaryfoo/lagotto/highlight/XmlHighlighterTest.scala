...
import io.github.binaryfoo.lagotto.highlight.XmlHighlighter.highlight

class XmlHighlighterTest extends LagoTest {

  "ANSI output format" should "highlight" in {
    val xml = contentsOf("one.xml")
    val highlighted = highlight(xml, AnsiMarkup)
  ...
  "HTML output format" should "highlight" in {
    val xml = contentsOf("one.xml")
    val highlighted = highlight(xml, HtmlMarkup)
  ...
  "Null output format" should "do nothing" in {
    val xml = contentsOf("one.xml")
    val highlighted = highlight(xml, NotMarkedUp)
